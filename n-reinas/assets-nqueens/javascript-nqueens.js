...
var NQUEENS = {
    ...
    init : function(runs) {
        ...
        var queens = parseInt(document.getElementById("queensnumber").value, 10),
            ...
            percentageCatastrophe =  parseInt(document.getElementById("percentagecatastrophe").value, 10),
            percentageFreeCatastrophe =  parseInt(document.getElementById("percentagefreecatastrophe").value, 10),
            maxGenerations =  parseInt(document.getElementById("maxgenerations").value, 10);

        runs = parseInt(runs, 10);

        if (runs < 1 || runs > 100) {
            this.util.log("Valor ingresado inválido.");
            validInput = false;
        }

        if (queens < 4 || queens > 128) {
            this.util.log("¡Valor ingresado inválido! Se permiten reinas de 4 a 128.");
            validInput = false;
        }
        ...
        if (validInput) {
            ...
            for (var i = 0; i < runs; i++) {
                ...
                var data = this.geneticAlgorithm.run(queens, populationSize, percentageCatastrophe, percentageFreeCatastrophe, maxGenerations);
                time = new Date().getTime() - time;

                if (data.result) {
                    this.util.log("[Ejecución #"+NQUEENS.runs+"] Solución encontrada para  "+queens+"-Reinas después de "+data.lastGeneration+" generaciones. ("+time+" ms, "+Math.ceil((time/data.lastGeneration))+" ms/gen) ");
                    
                    this.util.log(this.util.generateHTMLforIndividual(data.solutionIndividual), "solutions");
                    
                    this.util.log("[Ejecución #"+NQUEENS.runs+"]", "solutions");
                } else {
                    this.util.log("[Ejecución #"+NQUEENS.runs+"] Solución NO encontrada para "+queens+"-Reinas después de "+data.lastGeneration+" generaciones. ("+time+" ms, "+Math.ceil((time/data.lastGeneration))+" ms/gen) ");
                }
    ...
    geneticAlgorithm : {
        ...
        run : function(queensNumber, _populationSize, _percentageFreeCatastrophe, _percentageCatastrophe, _maxGenerations) {
            var i;

            var populationSize = _populationSize || 100; //queensNumber * 2;
            var maxGenerations = _maxGenerations || 1000;
            var percentageCatastrophe = _percentageCatastrophe || 5;
            var percentageFreeCatastrophe = parseFloat((_percentageFreeCatastrophe || 1) / 10);
            var catastropheFreeGenerations = percentageFreeCatastrophe * maxGenerations;
            ...
            for (currentGeneration = 1; currentGeneration <= maxGenerations ; currentGeneration++) {
                // Catastrophe.
                if ((currentGeneration > catastropheFreeGenerations) && (NQUEENS.util.random(1, 100) > percentageCatastrophe)) {
                ...
                var strPoblacion = "", _util = NQUEENS.util;
                ...
                for (i = 0; i < individuals.length; i++) {
                    ...
                    if (individuals[i].fitness === 0 && solutionFound === false) {
                        ...
                        strPoblacion += "<span class='fitness'>Individuo "+i+": "+_util.array2STR(individuals[i].chromosome)+" (Fitness: "+individuals[i].fitness+")</span>";
                    } else {
                        strPoblacion += "Individuo "+i+": "+_util.array2STR(individuals[i].chromosome)+" (Fitness: "+individuals[i].fitness+")";
                    }
                ...
                _util.log("Individuos: <br>"+strPoblacion);
                _util.log("Generación #"+currentGeneration+":");
