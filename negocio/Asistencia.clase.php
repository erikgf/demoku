    ...
    public function listarFechasDetalle($fecha){
        try {
           ...
           $sql = "SELECT
                ...
                (SELECT to_char(fecha_hora_registro,'HH24:MI:SS') FROM tbl_asistencia_envio_detalle WHERE cod_envio_cabecera = ac.cod_envio_cabecera AND dni_asistencia = ad.dni_asistencia AND tipo_registro = 'E' LIMIT 1) as ingreso,
                (SELECT to_char(fecha_hora_registro,'HH24:MI:SS') FROM tbl_asistencia_envio_detalle WHERE cod_envio_cabecera = ac.cod_envio_cabecera AND dni_asistencia = ad.dni_asistencia AND tipo_registro = 'S' LIMIT 1) as salida,
                (SELECT DATE_PART('hour',cast(fecha_hora_registro as time) - cast(tu.desde as time)) * 60 + DATE_PART('minute',cast(fecha_hora_registro as time) - cast(tu.desde as time)) FROM tbl_asistencia_envio_detalle WHERE cod_envio_cabecera = ac.cod_envio_cabecera AND dni_asistencia = ad.dni_asistencia AND tipo_registro = 'E' LIMIT 1) as nivel_tardanza,
                ac.idpuntoacceso, 
